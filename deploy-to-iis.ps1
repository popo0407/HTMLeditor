...
if ($nssmInstall.ExitCode -eq 0) {
    ...
    # 環境変数を明示的に設定
    & $nssmPath set $ServiceName AppEnvironmentExtra "PYTHONPATH=$BackendDeploy"
    & $nssmPath set $ServiceName AppEnvironmentExtra "VIRTUAL_ENV=$venvPath"
    & $nssmPath set $ServiceName AppEnvironmentExtra "PATH=$venvScriptsPath;%PATH%"
    ...
    $testResult = Start-Process -FilePath $pythonExe -ArgumentList "-c", "import uvicorn; print('uvicorn import successful')" -Wait -PassThru -WindowStyle Hidden -WorkingDirectory $BackendDeploy
    if ($testResult.ExitCode -eq 0) {
        Write-ColorOutput Green "    ✓ Uvicorn インポート成功"
    } else {
        Write-ColorOutput Red "    ✗ Uvicorn インポート失敗"
    }
    ...
    try {
        ...
        if (Test-Path $stderrLog) {
            ...
            if ($errorContent) {
                Write-ColorOutput Red "  - エラーログの内容:"
