...
# PDF出力用 fetch参照CSSも同期
$publicPdfCss = ".\frontend\public\pdf.css"
...
"@
    ...
    # public/pdf.css も更新（HTMLエクスポート用 fetch参照）
...
"@
    ...
    $publicPdfSize = (Get-Item $publicPdfCss).Length
    ...
    Write-Host "   → HTMLエクスポート用: $publicPdfCss ($publicPdfSize bytes)" -ForegroundColor Gray
    ...
    $publicPdfTime = (Get-Item $publicPdfCss).LastWriteTime
    ...
    Write-Host "   HTMLエクスポート用更新: $($publicPdfTime.ToString('yyyy-MM-dd HH:mm:ss'))" -ForegroundColor Gray
} catch {
